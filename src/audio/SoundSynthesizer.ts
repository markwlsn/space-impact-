...
export class SoundSynthesizer {
  ...
  /** Authentic 80s/90s arcade coin insert 'ka-ching' */
  public playCoinInsert(): void {
    if (!this.ctx || this.isMuted) return;
    const now = this.ctx.currentTime;

    // Metallic chime high ping
    const osc1 = this.ctx.createOscillator();
    const osc2 = this.ctx.createOscillator();
    const gain = this.ctx.createGain();
    ...
    osc1.frequency.setValueAtTime(2637.02, now + 0.08); // E7

    osc2.type = 'square';
    osc2.frequency.setValueAtTime(3951.07, now);
  ...
  /** Soft terminal mechanical keyclick */
  ...
  /** Triumphant fanfare when player overtakes a high score ranking */
  public playRankUpFanfare(): void {
    if (!this.ctx || this.isMuted) return;
    const now = this.ctx.currentTime;
    const notes = [523.25, 659.25, 783.99, 1046.5]; // C5, E5, G5, C6
  ...
  /** Deep atmospheric swelling synth drone for cinematic intro */
  public playCinematicChord(freqs: number[] = [110, 164.81, 220, 329.63]): void {
    if (!this.ctx || this.isMuted) return;
    const now = this.ctx.currentTime;
